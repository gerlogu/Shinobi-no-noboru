...
class mainMenu extends Phaser.Scene{
    ...
    preload(){
        ...
        this.load.image('Settings'              , 'assets/main-menu/boton_settings.png');
        this.load.image('SettingsSelected'      , 'assets/main-menu/boton_settings_seleccionado.png');
    ...
    create(){
        ...
        if(game.waterfallSound){
            game.waterfallSound.volume = game.sfxVolume/5;
        }else if(!game.waterfallSound){
            game.waterfallSound = game.sound.add('Waterfall');
            game.waterfallSound.play({
                volume: game.sfxVolume/5,
                loop: true
            });
        }
        ...
        this.localGameButton = this.add.sprite(this.width/2, this.height/1.691,'play').setInteractive();
        this.localGameButton.displayWidth = 178;
        ...
        this.localGameButton.on('pointerup', function(){
           ...
           // game.waterfallSound.volume = 0;
            ...
            that.scene.get("mainMenu").time.addEvent({delay: 210, callback: function(){that.scene.start('localGameSettings');}, callbackScope:this, loop:false});
        });


        // this.OnlineGameButton = this.add.sprite(this.width/2, this.height/1.59,'Online-game').setInteractive();
        ...
        this.SettingsButton = this.add.sprite(this.width/2,this.height/1.505,'Settings').setInteractive();
        this.SettingsButton.displayWidth = 230;
        this.SettingsButton.scaleY= this.SettingsButton.scaleX;
        this.SettingsButton.setDepth(2000);
        this.SettingsButton.on('pointerup', function(){
            that.sound2.play();
            that.cameras.main.fadeOut(200);
            that.scene.get("mainMenu").time.addEvent({delay: 210, callback: function(){that.scene.start('settingsMenu');}, callbackScope:this, loop:false});
        });

        this.ControlsButton = this.add.sprite(this.width/2,this.height/1.347,'Controls').setInteractive();
        this.ControlsButton.displayWidth = 230;
        this.ControlsButton.scaleY= this.SettingsButton.scaleX;
        ...
        this.CreditsButton = this.add.sprite(this.width/2,this.height/1.223,'Credits').setInteractive();
        this.CreditsButton.displayWidth = 230;
        this.CreditsButton.scaleY= this.SettingsButton.scaleX;
        ...
        //   });
          ...
          this.anims.create({
            ...
            frames: [ { key: 'SettingsSelected'} ],
            frameRate: 10,
            repeat: -1
          });
          this.anims.create({
            key: 'SettingsUnselected',
            frames: [ { key: 'Settings'} ],
            frameRate: 10,
            repeat: -1
          });
        ...
        this.localGameButton.on('pointerover', function() {
            ...
            that.localGameButton.anims.play('play_confirmed');
        ...
        this.localGameButton.on('pointerout', function() {
            that.localGameButton.anims.play('play');
        ...
        this.SettingsButton.on('pointerover', function() {
            ...
            that.SettingsButton.anims.play('SettingsSelected');
            do{  //Reproducimos el sonido unicamente si no se ha reproducido antes, es decir, si acabamos de entrar con el raton al botón. Si ya llevamos un rato
                that.sound1.play();         //el sonido no se reproducirá gracias al booleano. El booleano vuelve a true, al sacar el ratón del botón.
                this.pointerOver = false;
            }while(this.pointerover);
        ...
        this.SettingsButton.on('pointerout', function() {
            that.SettingsButton.anims.play('SettingsUnselected');
        ...
        //     do{   //Reproducimos el sonido unicamente si no se ha reproducido antes, es decir, si acabamos de entrar con el raton al botón. Si ya llevamos un rato
        //         that.sound1.play({
        //             volume: game.sfxVolume
        //         });           //el sonido no se reproducirá gracias al booleano. El booleano vuelve a true, al sacar el ratón del botón.
        //         this.pointerOver = false;
        //     }while(this.pointerOver);
        // });
    ...
    InitSubmenus(){
        ...
        this.sound3.volume = game.sfxVolume;
        //this.sound3.volume = game.sfxVolume;
